import type {iBlockEntry, iSocialAccount} from './datatypes';
...
export default class BlockEntry {
    ...
    annotations: string[]
    urls: string[]
    social_accounts: Map<string,iSocialAccount[]>
    ...
    constructor(entry: iBlockEntry) {
        ...
        this.social_accounts = new Map<string,iSocialAccount[]>();
    ...
    get_primary_account(platform: string): iSocialAccount | undefined {
        if (! this.social_accounts.has(platform)) {
            return undefined
        }
    ...
    get_accounts(platform: string): iSocialAccount[] {
        if (! this.social_accounts.has(platform)) {
            return []
        }
